...
export async function POST(request: NextRequest) {
  console.log("[API Proxy] ===== VERIFY LINK CHALLENGE REQUEST START =====")
  ...
  try {
    ...
    if (!body.link || typeof body.link !== 'string' || !body.link.trim()) {
      console.error("[API Proxy] ERROR: Invalid request body - link is required")
      return NextResponse.json(
    ...
    const targetUrl = `${API_BASE_URL}v1/challenges/verify/link/${ucId}`
    ...
    console.log("[API Proxy] ===== VERIFY LINK CHALLENGE REQUEST SUCCESS =====")
