...
// Use challenges/parser API for resume scores (not auth API)
const API_BASE_URL = "https://elite-challenges-xp-c57c556a0fd2.herokuapp.com/"

export async function GET(request: NextRequest) {
  console.log("[API Proxy] ===== GET RESUME SCORES REQUEST START =====")
  ...
  try {
    ...
    const targetUrl = `${API_BASE_URL}v1/users/resume-scores`
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { 
          error: errorText || 'Failed to get resume scores',
    ...
    console.log("[API Proxy] ===== GET RESUME SCORES REQUEST SUCCESS =====")
